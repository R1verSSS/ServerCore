...
const { enqueue, pause, resume, skip, stop, leave, buildQueuePayload, buildMusicStatusPayload, buildVoiceDiagnosePayload, isMusicEnabled } = require('../services/musicService');
...
module.exports = {
  data: new SlashCommandBuilder()
    ...
    .addSubcommand(sub => sub.setName('diagnose').setDescription('Проверить окружение Discord Voice на хостинге')),
  async execute(interaction) {
    const sub = interaction.options.getSubcommand();
    await safeDefer(interaction, { flags: MessageFlags.Ephemeral });

    if (sub === 'status') return safeEdit(interaction, buildMusicStatusPayload(interaction.guildId));
    if (sub === 'diagnose') return safeEdit(interaction, buildVoiceDiagnosePayload());
