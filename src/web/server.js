...
function userLayout(title, body, auth, message = '', warning = '') {
  ...
  return `<!doctype html><html lang="ru"><head><meta charset="utf-8"/><meta name="viewport" content="width=device-width, initial-scale=1"/><title>${escapeHtml(title)}</title><style>:root{color-scheme:dark;--bg:#0f1117;--card:#171a23;--line:#2b3040;--text:#f5f6fa;--muted:#aeb4c2;--accent:#5865f2}*{box-sizing:border-box}body{margin:0;background:linear-gradient(135deg,#0f1117,#111827);color:var(--text);font-family:Inter,Arial,sans-serif}.app{display:grid;grid-template-columns:260px 1fr;min-height:100vh}.side{background:#0b0e15;border-right:1px solid var(--line);padding:18px;position:sticky;top:0;height:100vh}.brand{font-size:22px;font-weight:800;margin-bottom:10px}.user{color:var(--muted);font-size:13px;margin-bottom:18px}.side a{display:block;color:var(--muted);text-decoration:none;padding:10px 12px;border:1px solid var(--line);border-radius:12px;margin:8px 0;background:#10131b}.side a:hover{color:#fff;background:rgba(88,101,242,.12)}.logout{color:#ffb3b5!important}.main{min-width:0}.top{position:sticky;top:0;background:rgba(17,19,27,.92);border-bottom:1px solid var(--line);padding:18px 24px}.top h1{margin:0;font-size:24px}.content{max-width:1200px;margin:0 auto;padding:24px}.grid{display:grid;grid-template-columns:repeat(auto-fit,minmax(240px,1fr));gap:16px}.card{background:rgba(23,26,35,.94);border:1px solid var(--line);border-radius:16px;padding:18px}.stat{font-size:32px;font-weight:800}.muted{color:var(--muted);line-height:1.45}.ok{background:rgba(87,242,135,.12);border:1px solid rgba(87,242,135,.35);padding:12px;border-radius:12px;margin-bottom:16px}.warn{background:rgba(237,66,69,.12);border:1px solid rgba(237,66,69,.35);padding:12px;border-radius:12px;margin-bottom:16px}table{width:100%;border-collapse:collapse}th,td{padding:10px;border-bottom:1px solid var(--line);text-align:left;vertical-align:top}th{color:var(--muted)}.pill{display:inline-block;border:1px solid var(--line);padding:4px 8px;border-radius:999px;color:var(--muted);font-size:12px;margin:2px}.section{margin-top:16px}.code{font-family:Consolas,monospace;background:#0b0d12;border:1px solid var(--line);border-radius:10px;padding:10px;white-space:pre-wrap}button,input,select,textarea{width:100%;border:1px solid var(--line);border-radius:12px;background:#10131b;color:var(--text);padding:12px 14px;font:inherit;outline:none;transition:border-color .18s ease,box-shadow .18s ease,background .18s ease}input::placeholder,textarea::placeholder{color:#747b8c}input:focus,select:focus,textarea:focus{border-color:var(--accent);box-shadow:0 0 0 3px rgba(88,101,242,.16);background:#121622}textarea{min-height:140px;resize:vertical;line-height:1.5}select{appearance:none;background-image:linear-gradient(45deg,transparent 50%,#aeb4c2 50%),linear-gradient(135deg,#aeb4c2 50%,transparent 50%);background-position:calc(100% - 18px) 50%,calc(100% - 13px) 50%;background-size:5px 5px,5px 5px;background-repeat:no-repeat;padding-right:38px}.form-stack{display:grid;gap:14px;margin-top:18px}.form-grid{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:14px}.field{display:grid;gap:7px}.field>span{font-size:13px;font-weight:700;color:var(--muted)}.field-full{grid-column:1/-1}.actions{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:10px;margin-top:16px}.actions form{margin:0}.action-link{display:flex;align-items:center;justify-content:center;gap:8px;min-height:42px;padding:10px 14px;border:1px solid var(--line);border-radius:12px;background:#10131b;color:var(--text);text-decoration:none;font-weight:700;transition:background .18s ease,border-color .18s ease,transform .18s ease}.action-link:hover{background:rgba(88,101,242,.14);border-color:rgba(88,101,242,.65);transform:translateY(-1px)}button{border:0;background:var(--accent);font-weight:800;cursor:pointer;min-height:42px}button:hover{filter:brightness(1.08)}button:disabled{cursor:not-allowed;opacity:.55;filter:none}.submit-row{margin-top:2px}.helper{font-size:12px;color:var(--muted);line-height:1.45}@media(max-width:820px){.app{display:block}.side{position:static;height:auto}.content{padding:16px}.form-grid,.actions{grid-template-columns:1fr}}</style></head><body><div class="app"><aside class="side"><div class="brand">🌐 ServerCore User</div><div class="user">${escapeHtml(auth?.label || auth?.userId || 'user')}<br/><span class="pill">${escapeHtml(auth?.userId || '')}</span></div>${nav}<a class="logout" href="/logout">Выйти</a></aside><div class="main"><div class="top"><h1>${escapeHtml(title)}</h1></div><main class="content">${message ? `<div class="ok">${escapeHtml(message)}</div>` : ''}${warning ? `<div class="warn">${escapeHtml(warning)}</div>` : ''}${body}</main></div></div></body></html>`;
}
...
function startWebPanel(client) {
  ...
  app.get('/my-tickets', (req, res) => {
    ...
    const form = `<div class="card"><h2>➕ Создать тикет</h2><p class="muted">Укажи тип, приоритет и подробное описание. После отправки бот создаст закрытый Discord-канал для общения с администрацией.</p><form class="form-stack" method="post" action="/user-actions/ticket"><div class="form-grid"><label class="field"><span>Тип обращения</span><select name="templateId"><option value="support">Поддержка</option><option value="bug">Баг / ошибка</option><option value="report">Жалоба</option><option value="shop">Магазин / покупка</option><option value="other">Другое</option></select></label><label class="field"><span>Приоритет</span><select name="priority"><option value="low">Низкий</option><option value="normal" selected>Обычный</option><option value="high">Высокий</option><option value="urgent">Срочный</option></select></label><label class="field field-full"><span>Краткая тема</span><input name="summary" maxlength="100" required placeholder="Например: не получается получить роль"/></label><label class="field field-full"><span>Подробное описание</span><textarea name="details" required placeholder="Что произошло, где и когда? Что уже пробовал сделать?"></textarea><small class="helper">Чем подробнее описание, тем быстрее администрация сможет помочь.</small></label></div><button class="submit-row" type="submit">🎫 Создать тикет</button></form></div>`;
  ...
  app.get('/my-topics', (req, res) => {
    ...
    const form = `<div class="card"><h2>➕ Создать forum-тему</h2><p class="muted">Покупки не нужны. Достаточно выбрать тип темы, указать название и описание. Бот создаст forum-пост в подходящем канале.</p><form class="form-stack" method="post" action="/user-actions/topic"><div class="form-grid"><label class="field field-full"><span>Тип темы</span><select name="type">${typeOptions}</select></label><label class="field field-full"><span>Название темы</span><input name="title" maxlength="90" required placeholder="Например: нужна помощь с проектом"/></label><label class="field field-full"><span>Описание</span><textarea name="body" required placeholder="Опиши вопрос, идею, проект или тему для обсуждения."></textarea><small class="helper">Тема появится в подходящем Forum-канале Discord.</small></label></div><button class="submit-row" type="submit">🧵 Создать тему</button></form></div>`;
