...
function buildMainMenuPayload() {
  ...
  const select = new StringSelectMenuBuilder()
    ...
    .addOptions(
      ...
      { label: 'Веб-панель', value: 'webpanel', emoji: '🌐', description: 'Ссылка, вход, аккаунт пользователя' },
...
function actionButtons(section) {
  ...
  const map = {
    ...
    webpanel: [
      new ButtonBuilder().setCustomId('menu:quick:webpanel').setLabel('Веб-панель').setEmoji('🌐').setStyle(ButtonStyle.Primary),
      new ButtonBuilder().setCustomId('webpanel:account').setLabel('Аккаунт').setEmoji('🔐').setStyle(ButtonStyle.Secondary),
      new ButtonBuilder().setCustomId('webpanel:code').setLabel('Код входа').setEmoji('🔑').setStyle(ButtonStyle.Secondary),
      back
    ],
    economy: [
      new ButtonBuilder().setCustomId('menu:quick:daily').setLabel('Daily').setEmoji('🎁').setStyle(ButtonStyle.Success),
...
function buildSectionPayload(section) {
  const map = {
    ...
    webpanel: {
      title: '🌐 Веб-панель',
      description: 'Личный кабинет пользователя и админ-панель ServerCore в браузере.',
      fields: [
        { name: 'Ссылка', value: 'Открыть веб-панель можно по закрепленной кнопке ниже или через `/webpanel open`.', inline: false },
        { name: 'Пользовательский вход', value: '`/webaccount password` — задать пароль\n`/webaccount login-code` — получить одноразовый код\n`/webaccount status` — проверить аккаунт', inline: false },
        { name: 'Что доступно пользователю', value: 'Профиль, баланс, история операций, инвентарь, тикеты, ивенты и покупки.', inline: false }
      ]
    },
    economy: {
